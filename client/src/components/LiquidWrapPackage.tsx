    ...
    >
      <div>
        <div className="flex justify-between items-center mb-2">
          <h3 className="font-heading text-lg font-bold text-gray-800">{title}</h3> {/* Updated text color for visibility */}
        </div>
        <p className="text-gray-600 mb-4">{description}</p>
        <p className="text-gray-800 font-semibold">{price}</p> {/* Updated text color for visibility */}
      </div>
      ...
      <div className={`p-3 rounded-md mb-4 ${isHighlighted ? 'bg-primary bg-opacity-5' : 'bg-gray-50'}`}>
        <div className="flex items-start mb-2">
          ...
          <div>
            <p className="font-semibold text-gray-800">{productType}</p> {/* Updated text color for visibility */}
